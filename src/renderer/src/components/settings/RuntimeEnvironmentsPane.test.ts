...
describe('RuntimeEnvironmentsPane host details', () => {
  ...
  // Why (B3): the Connect handler must go through the shared replay+refresh
  // primitive so tombstones are replayed and lineage is fetched once against the
  // connected host. There is no exported unit seam for the inline handler, so
  // this asserts the wiring at the source level — guarding against a regression
  // back to the bare refresh (no replay) or the per-repo N+1 lineage.
  it('Connect replays then refreshes projects via the shared primitive, not the per-repo N+1', async () => {
    ...
    expect(source).toContain('replayThenRefreshRuntimeEnvironment(useAppStore, environment.id)')
